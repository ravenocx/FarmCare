...
class MedicationDeliveryUpdate extends DuskTestCase
{
    /**
     * A Dusk test example.
     */
    public function testExample(): void
    {
        $this->browse(function (Browser $browser) {
            ...
            $browser->visit('/medicine/status/44')
                    ...
                    ->assertSee('Received Order')
                    ->press('Received Order')
                    ->pause(2000) // wait for the status to update
                    ->assertSee('Complete'); // Check if status is now 'Complete'
        });
